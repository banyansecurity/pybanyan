...
from banyan.model.user_device import TrustScore, TrustType
from tests.parsing import load_testdata


class TrustScoreV2ParserTest(unittest.TestCase):
    def test_score(self):
        ts: List[TrustScore] = TrustScore.Schema().loads(load_testdata("tests/data/trust_scores.json"), many=True)
        self.assertEqual(2, len(ts))
        self.assertEqual('C02TQ08VHF1R', ts[0].trust_id)
        self.assertEqual(TrustType.DEVICE, ts[0].trust_type)
        self.assertEqual('932f0800-556b-4296-a07c-92971a5c87c4', ts[0].factors.score_id)
        self.assertEqual('C02TQ08VHF1R', ts[1].trust_id)
        self.assertEqual(TrustType.EXTERNAL, ts[1].trust_type)
...
class TrustFactorV2ParserTest(unittest.TestCase):
    def test_factors(self):
        ts: TrustFactorsV2 = TrustFactorsV2.Schema().loads(load_testdata("tests/data/trust_factors.json"))
